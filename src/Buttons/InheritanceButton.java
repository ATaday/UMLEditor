...
import Elements.Element_Inheritance;
import Panels.StatisticsArea;

public class InheritanceButton extends Button {
	private static final long serialVersionUID = 1L;

	public InheritanceButton(final Gui inGui) {
		super();
		...
		this.setAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				if (ClassList.Get().CheckIfClassesPossible(Global.INHERITANCE_BASE)) {
					...
					ElementList.Get().add(
							new Element_Inheritance(ClassList.Get().GetCurrentChosen(), ClassList.Get()
									.GetCurrentMate()));

					child.connectionMethods.add(Global.INHERITANCE_CHILD);
					parent.connectionMethods.add(Global.INHERITANCE_PARENT);
		...
		this.setIcon(new ImageIcon("img/inheritance.jpg"));
	}
}